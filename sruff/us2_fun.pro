Function US2_fun, l, u, v, eps, status = st
...
;	kernel JJ2_ARG (in SRUFF).  Also calls CAST, DEFAULT and TYPE from MIDL.
...
;-
	...
	lh = fix(l)/2
	if fix(l) mod 2 eq 0 then res = -(-1)^lh*lh*beselj(l*u,lh) else res = 0.
    endif else res = l*u*(0.5*((l eq 2) + (l eq -2)) + $
    1/!dpi*Romberg('JJ2_arg',!dpi*[-1,1],eps,par=[l,u,v],/rel,stat=st,try=3))
