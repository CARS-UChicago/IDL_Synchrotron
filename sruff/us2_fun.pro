Function US2_fun, l, u, v, eps, verify = ver, status = st
...
;		4.2
; PURPOSE:
;		Calculates the sum /Sigma{-/inf,/inf}{n*J_n(lu)*J_2n+l(lv) which is used
;		in SR undulator calculations.
; CATEGORY:
;		Mathematical, SR specific
; CALLING SEQUENCE:
;		Result = US2_FUN (L, U, V [, EPS] [, STATUS = ST])
; INPUTS:
;	L
;		Integer scalar.
;	U
...
;		kernel JJ2_ARG (in SRUFF).  Also calls CAST, DEFAULT, TOLER and TYPE
;		from MIDL.
...
;-
	...
	if abs(v) lt eps then begin
		lh = ll/2
		if ll mod 2 eq 0 then res = -(-1)^lh*lh*beselj(l*u,lh) else res = $
		-(-1)^lh*(ll*v/4.)*((ll-1)*beselj(ll*u,lh) + (ll+1)*beselj(ll*u,lh+1))
	endif else res = l*u*(0.5d*((l eq 2) + (l eq -2)) + 1/!dpi* $
	Romberg('JJ2_arg',!dpi*[-1,1],eps,par=[l,u,v],/rel,stat=st,try=3,ver=ver))
